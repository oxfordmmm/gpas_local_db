...
def db_session(postgresql) -> scoped_session:
    ...
    ## add countries
    session.add(models.Country(code="GBR", name="United Kingdom", code2="UK", lat=54.0, lon=-2.0))
    ...
    sample_detail_types = [
        ...
        {"code": "pre_sequence_concentration", "description": "", "value_type": "float"},
        {"code": "dilution_post_initial_concentration", "description": "", "value_type": "bool"},
    ...
    ## add runs
    session.add(models.Run(code="Run1", site="SiteA", sequencing_method="Illumina", machine="Machine1"))
    session.add(models.Run(code="test1", run_date=date.fromisoformat("2024-01-01"), site="Oxford", sequencing_method="Illumina", machine="test_m1", user="blah", number_samples=2, flowcell="fc2", passed_qc=True, comment="test_comment"))
