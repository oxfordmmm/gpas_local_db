...
## Summary.csv from GPAS upload via CLI

The CLI program also allows for the upload of a `summary.csv`, output from the GPAS system. You must also provide the `mapping.csv` generated by the GPAS CLI when you uploaded the `FASTQ` files to the GPAS system. Use the following command to upload the `summary.csv` into the Local Hospital Database.
...
- [ ] Try uploading a `summary.csv` where all the samples are in the database. You should get a clean upload.
- [ ] Try removing a row from the `mapping.csv`, you should get an error.
- [ ] Try removing the `sample_name` from one of the rows in the `summary.csv`, you should get an error.
...
## Mutation.csv from GPAS upload via CLI

The CLI program also allows for the upload of a `mutation.csv`, output from the GPAS system. You must also provide the `mapping.csv` generated by the GPAS CLI when you uploaded the `FASTQ` files to the GPAS system. Use the following command to upload the `mutation.csv` into the Local Hospital Database.

```bash
gpaslocal mutation mutation.csv mapping.csv --dryrun
```

Replacing gpaslocal with the name of you executable, `mutation.csv` and `mapping.csv` with the name and location of your `mutation.csv` and `mapping.csv` respectively.
